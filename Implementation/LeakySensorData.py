...
class LeakyScenario(ScenarioPreprocessing):
    ...
    def __load(self):
        ...
        fault_info_file = list(filter(lambda z: z.endswith(".xlsx"), os.listdir(os.path.join(self.path_to_data, f"scenario{self.scenario_id}", "Leakages"))))[0]
        df_fault_info = pd.read_excel(os.path.join(self.path_to_data, f"scenario{self.scenario_id}", "Leakages", fault_info_file), sheet_name="Info", engine='openpyxl')
...
def enumerate_all_leaky_scenarios(path_to_scenarios):
